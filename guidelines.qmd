...
In **[Phase One](#phase-one)** we will aim to develop an updated fern and lycophyte classification at the rank of genus and above—this is effectively a direct update to PPG I.
In **[Phase Two](#phase-two)**, the plan for which is still being developed, we will expand our classification to include the species rank—at the end of this phase we aim to have a classification that includes a full list of extant fern and lycophyte species.
...
First and foremost, **we are moving our discussion and proposal platform away from deliberation over email to a PPG-wide GitHub-based platform** (described [below](#github-interface-for-communication-and-coordination); see also the [GitHub Guide](https://pteridogroup.github.io/github.html)).
...
Secondly, in conjunction with this new platform, we are moving away from the committee-based model^[For [Phase One](#phase-one) at least. A committee-based model may still be used for [Phase Two](#phase-two)] (each committee made a proposal for the classification of their focal taxon, and those proposals were then voted upon by the full PPG community).
...
- [The PPG website](https://pteridogroup.github.io) provides documentation about the maintenance of PPG and serves as a [user-friendly portal to the data](ppg.qmd).

- [The PPG GitHub repo](https://github.com/pteridogroup/ppg) houses the [raw data](https://github.com/pteridogroup/ppg/releases) and is used as a forum for discussing [taxonomic proposals](https://github.com/pteridogroup/ppg/issues).

- [World Flora Online (WFO)](https://about.worldfloraonline.org/) provides an interface to edit the data, [Rhakhis](https://list.worldfloraonline.org/rhakhis/ui/index.html).
For more information, see the [World Flora Online section](wfo.qmd).

## Phase One

The goal of Phase One is to update the PPG I [-@ppg2016] genus-and-above classification of ferns and lycophytes.

### Intended products

The primary product of Phase One will be a peer-reviewed publication (PPG II) with all PPG contributors included as authors, similar to PPG I [-@ppg2016].
This publication will also include elements of Phase Two, such as an online supplement providing the data in a standard format ([Darwin Core](https://dwc.tdwg.org/terms/#taxon)).
In addition to a different [decision-making process](#voting), this publication will differ from PPG I [-@ppg2016] in that we will explicitly discuss minority opinions (defined as those with at least ⅙ votes against).
...
GitHub is easy to use, the only requirement being that each participant needs to create a free account; detailed instructions are available in the [GitHub Guide](https://pteridogroup.github.io/github.html#setup).
...
Much of the success of Phase One will depend on PPG members making clear and informative proposals.
...
For example, if one is advocating for a name change based upon the non-monophyly of a taxon recognized in PPG I [-@ppg2016], the phylogeny demonstrating non-monophyly needs to be published.
2. **Each proposal should take PPG I [-@ppg2016] as the starting point** (i.e., only _changes_ to PPG I should be proposed).
3. Proposals to recognize nothotaxa should only be made if the parent taxa are already recognized.
4. Each proposal should be cohesive and self-contained—try to avoid compound proposals where possible.
...
5. Anyone is welcome to make a proposal for any taxon, but it would be collegial to allow the researchers most closely associated with the name change "first dibs", so to speak.
These people are also probably in the best position to write a comprehensive proposal.
6. Each proposal should be comprehensive: it should include all relevant details to allow other PPG members to make an informed decision about the proposal.
...
7. **Please only submit proposals that you personally think deserve recognition in PPG and can vouch for during the discussion.**
It is not necessary to submit a proposal simply because it is a nomenclaturally valid alternative.
There are countless alternative ways to devise the taxonomy; we should only spend our time voting on those that our members feel are worthy of recognition.
If you don't feel strongly either way about a particular taxonomic treatment but would like to ask what others think, please start a [discussion](https://github.com/pteridogroup/ppg/discussions).
...
Our initial database is based on Michael Hassler's [World Ferns](https://www.worldplants.de/world-ferns/ferns-and-lycophytes-list) list (thank you Michael!). Joel Nitta [has written a script](https://github.com/pteridogroup/ppg-import) that converts this list into Darwin Core format. Note that this database includes various changes relative to PPG I [-@ppg2016] made by Michael, but none of these will be considered official until they have been approved via the process outlined [above](#voting).

As of 2025-06, the data have been imported into [World Flora Online](https://about.worldfloraonline.org/) and are maintained using their online taxonomic database editor, [Rhakhis](https://list.worldfloraonline.org/rhakhis/ui/index.html).

## Phase Two

The major difference between Phase One and Phase Two is that Phase One is at the genus (and higher) level, whereas Phase Two is at the species (and higher) level^[Infraspecific taxa will also be treated.].

Note that the proposal and voting process initiated during Phase One will continue indefinitely, regardless of the start of Phase Two.

### Intended products

The primary product of Phase Two will be a database of species for each of the higher taxa recognized as a result of Phase One.
...
All editing of the data will take place on [World Flora Online](wfo.qmd)'s online taxonomic database editing system, [Rhakhis](https://list.worldfloraonline.org/rhakhis/ui/index.html).

If you wish to participate in Phase II, please see the [Curator's Guide](curators.qmd).

## Versioning

We anticipate ongoing updates to the taxonomic database.
Therefore, rather than release a major new PPG (e.g., PPG 3, PPG 4, etc.) only every few years, [semantic versioning](https://semver.org/) will be used to release updates on a more frequent and granular scale.
Versioning will follow a numbering scheme with three digits (e.g., v1.0.0) where changes below genus will be increments of the last digit (e.g., v1.0.1), changes at intermediate levels (genus to order) will be the middle digit (e.g., v1.1.0), and dramatic changes (order or higher, or a completely different data format) will be the first digit (e.g., v2.0.0).

PPG v1.0.0 is something of an exception to the above. This refers to the data as specified in PPG I [-@ppg2016], and we have no intention of issuing changes within v1 (e.g., v1.1.0). Rather, the next update will be v2.0.0, to be released at the completion of [Phase One](#phase-one).
...
The PPG community will be notified before any major updates.
Only changes that have been approved by the voting system will be included in updates.
A DOI will be provided for each version via [Zenodo](https://zenodo.org/).

## Reference Library

A [Zotero library](https://www.zotero.org/groups/5043626/pteridogroup/library) is being maintained to store references relevant to PPG.
If you are interested in helping to maintain the reference library, please [contact a member of the coordinating committee](#coordinating-committee).
